...
export async function createEvent(input: CreateEventInput): Promise<CreateEventResult> {
  try {
    ...
    console.error(`[calendar] createEvent threw: ${formatDbError(error)}`);
    return {
      ok: false,
      reason: "failed",
...
export async function deleteEvent({
  ...
  try {
    ...
    console.error(`[calendar] deleteEvent failed: ${formatDbError(error)}`);
    return { ok: false };
  }
}
...
export async function test(
  ...
  try {
    ...
    console.error(`[calendar] test failed: ${formatDbError(error)}`);
    await recordTestResult({
      organizationId,
      provider: CALENDAR_PROVIDER,
      status: "error",
