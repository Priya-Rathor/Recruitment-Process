...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error(`[api] jobs list failed: ${formatDbError(error)}`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error(`[api] job create failed: ${formatDbError(error)}`);
...
export async function replaceQuestions({
  ...
  for (const [key, table] of [
    ...
    if (deleteError) {
      console.error(`[api] clearing ${table} failed: ${formatDbError(deleteError)}`);
    ...
    if (insertError) {
      console.error(`[api] inserting ${table} failed: ${formatDbError(insertError)}`);
