...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error) {
      console.error(`[api] job fetch failed: ${formatDbError(error)}`);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error) {
      ...
      console.error(`[api] job update failed: ${formatDbError(error)}`);
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error(`[api] job archive failed: ${formatDbError(error)}`);
