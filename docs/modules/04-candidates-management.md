...
Purpose
This module is the single source of truth for candidate identity and profile data, regardless of how the candidate entered the system.
Key Features / What This Module Manages
	•	Candidate intake from career page, resume upload, email, referral, job boards, agency database, or manual entry
	•	Structured candidate profile: contact, experience, skills, current role/company, location, salary, notice period
	•	Deduplication against existing candidates (by email/phone)
	•	Natural-language candidate search
	•	Candidate detail view showing full history across jobs/applications
Main Routes
	•	/candidates
	•	/candidates/new
	•	/candidates/[id]
Core Data Model
	•	candidates (id, organization_id, name, email, phone, location, current_company, current_role, total_experience_years, skills, expected_salary, notice_period_days, source, resume_url, created_at)
	•	candidate_duplicates (candidate_id, duplicate_of_id, matched_on, status)
UI / Operational Flow
	•	Candidate enters via any source -> raw data captured
	•	AI structures free-text/resume content into profile fields (see Module 6 for resume-specific parsing)
	•	System checks for duplicates before creating a new candidate record
	•	Recruiter can search using natural language instead of stacking filters
How This Module Connects To The Others
Depends on:
	•	Module 1: tenant resolution + role checks
Provides for later modules:
	•	candidates, candidate_duplicates — read by Module 5 (applications), Module 6 (resume parsing writes back here), Module 7 (matching), Module 8 (screening), Module 9, Module 12 (client submissions)
How AI Is Used In This Module
AI converts messy candidate information into structured data - for example turning a raw resume line ("Rahul Sharma, Software Engineer at Infosys...") into name, experience, current role, skills, current company, and location. AI also powers natural-language search, letting a recruiter type "Show me Java candidates in Gurgaon with 4-7 years experience and less than 45 days notice" instead of configuring ten filters. AI never overwrites an existing verified field silently - conflicting extractions are surfaced for recruiter confirmation, following the same propose-then-review pattern used throughout the product.
Role Permissions
Permission
Owner
Admin
...
Yes
Yes
Yes (read-only results)
Build Now (MVP)
	•	Candidate CRUD across all intake sources
	•	Deduplication check
	•	AI-assisted structuring of free-text profile data
	•	Natural-language search (basic)
Build Later (Not MVP)
	•	LinkedIn/job-board direct ingestion
	•	Advanced fuzzy deduplication across historical records
Testing Checklist
	•	Duplicate detection catches same email/phone across different sources
	•	Natural-language search returns results matching literal filter equivalents
	•	AI-structured fields are editable and persist recruiter corrections
	•	Tenant isolation on candidate list/detail/search
Claude Implementation Prompt — Module 4: Candidates Management
Save this specification as /docs/modules/04-candidates-management.md, then give Claude the prompt below:
Build Module 4: Candidates Management for my multi-tenant recruitment SaaS.
 
Before writing any code, read: the global design system, /docs/modules/04-candidates-management.md, and every previously built module's code/spec (earlier modules define tables, RLS conventions, and the AI Service Layer this module must reuse).
...
=================================================================
- GET /api/candidates — list, organization-scoped, supports pagination + the module's standard filters
- POST /api/candidates — create a new record (validates payload server-side, ignores any client-supplied organization_id)
- GET /api/candidates/:id — fetch one record the caller's organization owns
- PATCH /api/candidates/:id — partial update, role-checked
- DELETE /api/candidates/:id — soft-delete/archive where applicable, role-checked
- POST /api/candidates/:id/ai-action — invokes the module's AI Service Layer function (see AI Integration section) and returns structured, unsaved suggestions for review
...
=================================================================
Purpose: This module is the single source of truth for candidate identity and profile data, regardless of how the candidate entered the system.
...
1. Candidate enters via any source -> raw data captured
2. AI structures free-text/resume content into profile fields (see Module 6 for resume-specific parsing)
3. System checks for duplicates before creating a new candidate record
4. Recruiter can search using natural language instead of stacking filters
...
Create/edit candidate | Yes | Yes | Yes | No
View all candidates | Yes | Yes | Yes | Yes
Use natural-language search | Yes | Yes | Yes | Yes (read-only results)
...
Simple UI Template — Module 4: Candidates Management
